...
const EARN_COOLDOWN_MS = 9 * 60 * 1000; // compatível com sessão de 10 min do front
...
async function earn(req, res, next) {
  try {
    ...
    // Cooldown anti-cheat
    const now = Date.now();
    if (user.lastEarnAt && (now - user.lastEarnAt.getTime()) < EARN_COOLDOWN_MS) {
...
async function spend(req, res, next) {
  try {
    ...
    const spendAmount = Math.max(0, Number(amount) || 2);
    const user = await User.findById(req.user._id);
    if (!user) return next(new AppError('Usuário não encontrado.', 404));

    if (user.tokens < spendAmount) {
      return next(new AppError(
        `Saldo insuficiente. Você tem ${user.tokens} tokens e precisa de ${spendAmount}. ` +
    ...
    const updated = await User.findOneAndUpdate(
      { _id: user._id, tokens: { $gte: spendAmount } },
      { $inc: { tokens: -spendAmount, totalTokensSpent: spendAmount, viewersReceived: 1 } },
      { new:true }
    );
    if (!updated) return next(new AppError('Saldo insuficiente.', 402));

    return res.json({ success:true, spent: spendAmount, newBalance: updated.tokens });
  } catch(err) { next(err); }
}
